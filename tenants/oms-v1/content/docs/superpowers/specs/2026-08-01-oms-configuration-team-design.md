...
- Team-member CRUD: Paylocity ID, name, title, home location,
  target hours, active state, consecutive-off exemption, and notes.
  Permanent unavailable days are edited as rotation `OFF` cells (schema v4).
- Ordered role preferences: role, rank/weight, burnout threshold, auto-assign;
  these supersede the redundant home-department field as of schema v4.
