...
| **SP2** | **12-factor envelope + document API** *(gated on SP1's conformance report — see §3.8)* | FastAPI + Postgres + Alembic + Entra OIDC + Docker Compose + Caddy. Frontend swaps `SchedulerStore` from IndexedDB to API-backed; Postgres becomes authoritative. **← ROLLOUT MILESTONE** |
...
**Track D landed upstream on 2026-08-04.** `4d1de56` cut the OMS over to
**Approach B** — departments nest their roles and default needs, a new
`src/model/omsSelectors.js` mediates access, and `buildSeed`, `omsMutations`,
`generate`, and `activeNeeds` were reworked to match. `omsPersistence.js` bumped
`SCHEMA` from 2 to 3, so the persisted document shape changed with it. `73c63e0`
rewrote HANDOFF.md around the new state. The earlier concern that this work was
unpushed and outside review is resolved.

Two facts about that cutover matter to this spec:
...
The lock screen was built and merged on 2026-07-27; `506dabc:src/ui/App.jsx`
carries `LoginScreen` wired into the shell. The OMS branch then rewrote
`App.jsx` wholesale on 2026-08-02 (`644fb2f`), **silently dropping it**, and it
was re-implemented from scratch on 2026-08-03 (`96594b6`). A full round of
duplicated work, caused by two tracks editing one file with no seam between them.
...
- **No wholesale file rewrites.** Additive changes to shared `src/` files; a
  ...
  screen.
- **The topology is fork-based.** `HinchK/scheduler` is upstream; Tom works in
  the `TomWCAH/scheduler` fork and integrates by PR. That is a workable model,
  but its characteristic failure is not the merge conflict — it is **work that
  never leaves the working copy.** Track D's current state (§4) is exactly that.
  Push early and often; an unpushed fork is invisible to review, CI, and backup.
...
SP2 adds `createOmsApiStore()` as a third implementation: same three methods,
same serialized payload, HTTP instead of IndexedDB. `OmsContext` selects the
implementation. No domain, engine, or UI code changes. The `schemaVersion` check
already in `deserializeOms` becomes the server-side contract version.
...
- **`app_user`** — Entra object ID, email, and the break-glass credential hash.
...
2. Entra ID login works end to end, and break-glass login works with the flag on.
...
2. **Entra tenant access** — Kasey and Tom are handling the app registration
   together. No longer externally blocked.
3. **HANDOFF Phase A** — complete. Tom is applying the lessons now (§4).
4. **Kenny Williams** — addressed in Tom's working copy.

**Closed 2026-08-04 (same day):** Track D's output reached upstream in `4d1de56`
and `73c63e0` — the Approach B cutover and the HANDOFF rewrite. The earlier
concern that this work existed only as an unpushed working copy no longer
applies.
...
**That prerequisite is now met.** Tom's Phase A corrections and the Approach B
cutover are merged upstream as of 2026-08-04, so SP1 has a stable target: build
the conformance fixtures against the **Approach B document shape (schema v3)**.
